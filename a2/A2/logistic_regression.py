...
class LogisticRegression:
    ...
    def _compute_loss_and_gradient(self, X, y, alpha_inverse=0, beta_inverse=0):
        ...
        softmax_value = softmax(np.matmul(X, self.parameters.T))
        nll = 0
        for i in range(N):
            # opt without error: nll -= np.log(softmax_value[i][y[i][0]])
            for k in range(self.num_classes):
                nll -= y_one_hot[i][k] * np.log(softmax_value[i][k])
        nll = nll / N
        # Calculate gradient
        grad = np.zeros(shape=(self.num_classes, D+1))
        for j in range(self.num_classes):
            for i in range(N):
                grad[j] += (softmax_value[i][j] - y_one_hot[i][j]) * X[i]
        grad = grad / N

        # Calculate NLP instead
        if alpha_inverse != 0 or beta_inverse != 0:

            c_inv = np.eye(D + 1)
            c_inv[0][0] = beta_inverse
            for i in range(D + 1):
                c_inv[i][i] = alpha_inverse
            # Calculate L2 term
            l2_term = -math.log(beta_inverse * alpha_inverse)
            for i in range(self.num_classes):
                l2_term += 0.5 * np.matmul(np.matmul(self.parameters[i].T, c_inv), self.parameters[i])
            nll += l2_term

            # Calculate gradient
            for i in range(self.num_classes):
                grad[i] += np.matmul(self.parameters[i], c_inv)
        # ====================================================
        return nll, grad
