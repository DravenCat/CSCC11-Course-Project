...
class GMM:
    ...
    def _e_step(self, train_X):
        ...
        for i in range(N):
            point_probs = np.empty(shape=(self.K,))
            for j in range(self.K):
                point_probs[j] = self.mixture_proportions[j] * \
                                 np.exp(-0.5 * (train_X[i] - self.centers[j])
                                        @ np.linalg.inv(self.covariances[j])
                                        @ (train_X[i] - self.centers[j])) / \
                                 np.sqrt(np.power((2 * np.pi), D)
                                         * np.linalg.det(self.covariances[j]))
            for j in range(self.K):
                probability_matrix[i][j] = point_probs[j] / np.sum(point_probs)
        # ====================================================

        assert probability_matrix.shape == (train_X.shape[0],
                                            self.K), f"probability_matrix shape mismatch. Expected: {(train_X.shape[0], self.K)}. Got: {probability_matrix.shape}"
    ...
    def _m_step(self, train_X, probability_matrix):
        ...
        # ====================================================
        # TODO: Implement your solution within the box
        sum_gamma = np.sum(probability_matrix, axis=0)
        for k in range(self.K):
            mixture_proportions[k] = sum_gamma[k] / N
            centers[k] = np.sum((probability_matrix[:, k:k+1] * train_X), axis=0) / sum_gamma[k]
            covariances[k] = ((train_X - centers[k]).T @ (probability_matrix[:, k:k+1] * (train_X - centers[k]))) / sum_gamma[k]

        # ====================================================

        assert centers.shape == (self.K,
                                 self.D), f"centers shape mismatch. Expected: {(self.K, self.D)}. Got: {centers.shape}"
        assert covariances.shape == (self.K, self.D,
                                     self.D), f"covariances shape mismatch. Expected: {(self.K, self.D, self.D)}. Got: {covariances.shape}"
        assert mixture_proportions.shape == (self.K,
                                             1), f"mixture_proportions shape mismatch. Expected: {(self.K, 1)}. Got: {mixture_proportions.shape}"
    ...
    def train(self, train_X, max_iterations=1000):
        ...
        """
        assert len(train_X.shape) == 2 and train_X.shape[
            1] == self.D, f"train_X should be a NxD matrix. Got: {train_X.shape}"
