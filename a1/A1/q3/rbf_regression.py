...
class RBFRegression():
    ...
    def predict(self, X):
        ...
        f(x) = w_0 + w_1 * b_0(x) + w_2 * b_1(x) + ... + w_K * b_{K-1}(x), 
        ...
        # Pad 1s for bias term
        num_sample = X.shape[0]
        x = np.ones((num_sample, 1))

        # Compute b_i(x) for each x
        for i in range(self.K):
            x = np.hstack((x, self._rbf_2d(X, i)))
    ...
    def fit_with_l2_regularization(self, train_X, train_Y, l2_coef):
        ...
        # Compute b_i(x) for each x
        for i in range(self.K):
            x = np.hstack((x, self._rbf_2d(train_X, i)))

        # calculate the optimal parameter using OLS
        self.parameters = np.matmul(
            np.matmul(
                np.linalg.inv(np.matmul(x.T, x) +
                              l2_coef * np.identity(self.K + 1)),
                x.T),
            train_Y)
